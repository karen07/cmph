            ...
            if (graph_node_is_critical(bmz->graph, i) && (!GETBIT(visited, i))) {
                if (c > 1.14)
                    restart_mapping = bmz_traverse_critical_nodes(
                        bmz, i, &biggest_g_value, &biggest_edge_value, used_edges, visited);
                else
...
static cmph_uint32 next_unused_edge(bmz_config_data_t *bmz, cmph_uint8 *used_edges,
...
cmph_uint32 bmz_packed_size(cmph_t *mphf)
{
    ...
    return (cmph_uint32)(sizeof(CMPH_ALGO) + hash_state_packed_size(h1_type) +
                         hash_state_packed_size(h2_type) + 3 * sizeof(cmph_uint32) +
                         sizeof(cmph_uint32) * data->n);
}
