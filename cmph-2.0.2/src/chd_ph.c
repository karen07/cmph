...
static cmph_uint8 chd_ph_bucket_insert(chd_ph_bucket_t *buckets, chd_ph_map_item_t *map_items,
                                       chd_ph_item_t *items, cmph_uint32 nbuckets,
...
cmph_uint8 chd_ph_mapping(cmph_config_t *mph, chd_ph_bucket_t *buckets, chd_ph_item_t *items,
                          cmph_uint32 *max_bucket_size)
{
    ...
    chd_ph_map_item_t *map_items =
        (chd_ph_map_item_t *)malloc(chd_ph->m * sizeof(chd_ph_map_item_t));
                                      ...
                                      cmph_uint32 max_bucket_size)
{
    chd_ph_sorted_list_t *sorted_lists =
        (chd_ph_sorted_list_t *)calloc(max_bucket_size + 1, sizeof(chd_ph_sorted_list_t));
    ...
    for (bucket_size = 1; bucket_size <= max_bucket_size; bucket_size++) {
        for (i = sorted_lists[bucket_size].buckets_list;
             i < sorted_lists[bucket_size].size + sorted_lists[bucket_size].buckets_list; i++) {
                                            ...
                                            cmph_uint32 size)
{
    ...
    if (chd_ph->keys_per_bin > 1) {
        ...
        {
            position = (cmph_uint32)((item->f + ((cmph_uint64)item->h) * probe0_num + probe1_num) %
                                     chd_ph->n);
    ...
    if (i != size) // Undo the placement
    {
        ...
        if (chd_ph->keys_per_bin > 1) {
            while (1) {
                ...
                position =
                    (cmph_uint32)((item->f + ((cmph_uint64)item->h) * probe0_num + probe1_num) %
                                  chd_ph->n);
                (chd_ph->occup_table[position])--;
                item++;
                i--;
    ...
    for (i = max_bucket_size; i > 0; i--) {
        ...
        while (sorted_lists[i].size != 0) {
            ...
            for (j = 0, non_placed_bucket = 0; j < sorted_lists[i].size; j++) {
                ...
                if (place_bucket_probe(chd_ph, buckets, items, probe0_num, probe1_num, curr_bucket,
                                       i)) {
...
// 					DEBUGP("BUCKET %u NOT PLACED\n", curr_bucket);
                    ...
                    buckets[non_placed_bucket + sorted_lists[i].buckets_list].u.bucket_id =
                        buckets[curr_bucket].u.bucket_id;
#ifdef DEBUG
...
cmph_uint8 chd_ph_searching(chd_ph_config_data_t *chd_ph, chd_ph_bucket_t *buckets,
                            chd_ph_item_t *items, cmph_uint32 max_bucket_size,
                            chd_ph_sorted_list_t *sorted_lists, cmph_uint32 max_probes,
                            cmph_uint32 *disp_table)
{
...
#ifdef DEBUG
static inline cmph_uint8 chd_ph_check_bin_hashing(chd_ph_config_data_t *chd_ph,
...
cmph_t *chd_ph_new(cmph_config_t *mph, double c)
{
    ...
    while (1) {
        ...
        sorted_lists =
            chd_ph_ordering(&buckets, &items, chd_ph->nbuckets, chd_ph->m, max_bucket_size);

        if (mph->verbosity) {
            fprintf(stderr, "Starting searching step\n");
        }

        searching_success = chd_ph_searching(chd_ph, buckets, items, max_bucket_size, sorted_lists,
...
cleanup:
    ...
    fprintf(stdout, "%u\t%.2f\t%u\t%.4f\t%.4f\t%.4f\t%.4f\n", chd_ph->m, load_factor,
            chd_ph->keys_per_bucket, construction_time, space_usage / (double)chd_ph->m,
