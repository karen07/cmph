...
static char *brz_copy_partial_fch_mphf(brz_config_data_t *brz, fch_data_t *fchf, cmph_uint32 index,
                                       cmph_uint32 *buflen);
...
static int brz_gen_mphf(cmph_config_t *mph)
{
    ...
    for (e = 0; e < brz->m; ++e) {
        ...
        if (memory_usage + keylen + sizeof(keylen) > brz->memory_availability) // flush buffers
        {
            ...
            for (i = 0; i < nkeys_in_buffer; i++) {
                ...
                h0 = hash(brz->h0, (char *)(buffer + memory_usage + sizeof(keylen1)), keylen1) %
                     brz->k;
        ...
        if ((brz->size[h0] == MAX_BUCKET_SIZE) ||
            (brz->algo == CMPH_BMZ8 &&
             ((brz->c >= 1.0) && (cmph_uint8)(brz->c * brz->size[h0]) < brz->size[h0]))) {
            free(buffer);
            free(buckets_size);
            return 0;
        }
    ...
    free(buckets_size);
    if (nflushes > 1024)
        return 0; // Too many files generated.
    // mphf generation
    if (mph->verbosity) {
        fprintf(stderr, "\nMPHF generation \n");
    }
    /* Starting to dump to disk the resulting MPHF: __cmph_dump function */
    nbytes = fwrite(cmph_names[CMPH_BRZ], (size_t)(strlen(cmph_names[CMPH_BRZ]) + 1), (size_t)1,
                    brz->mphf_fd);
                                   ...
                                   cmph_uint32 *fingerprint)
{
    ...
    DEBUGP("key: %s g[h1]: %u g[h2]: %u offset[h0]: %u edges: %u\n", key, brz->g[h0][h1],
           brz->g[h0][h2], brz->offset[h0], brz->m);
...
cmph_uint32 brz_packed_size(cmph_t *mphf)
{
    ...
    size = (cmph_uint32)(2 * sizeof(CMPH_ALGO) + 3 * sizeof(CMPH_HASH) +
                         hash_state_packed_size(h0_type) + sizeof(cmph_uint32) + sizeof(double) +
                         sizeof(cmph_uint8) * data->k + sizeof(cmph_uint32) * data->k);
// pointers to g_is
#if defined(__ia64) || defined(__x86_64__)
...
static cmph_uint32 brz_bmz8_search_packed(cmph_uint32 *packed_mphf, const char *key,
...
static cmph_uint32 brz_fch_search_packed(cmph_uint32 *packed_mphf, const char *key,
                                         cmph_uint32 keylen, cmph_uint32 *fingerprint)
{
    register CMPH_HASH h0_type = (CMPH_HASH)*packed_mphf++;
