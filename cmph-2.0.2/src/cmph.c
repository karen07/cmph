...
static int key_struct_vector_read(void *data, char **key, cmph_uint32 *keylen)
{
    ...
    *key = (char *)malloc(size);
    keys_vd_offset =
        ((cmph_uint64)cmph_struct_vector->position * (cmph_uint64)cmph_struct_vector->struct_size) +
                                                 ...
                                                 cmph_uint32 nkeys)
{
    cmph_io_adapter_t *key_source =
        cmph_io_struct_vector_new(vector, struct_size, key_offset, key_len, nkeys);
