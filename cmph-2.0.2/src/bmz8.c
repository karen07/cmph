...
static cmph_uint8 bmz8_traverse_critical_nodes_heuristic(bmz8_config_data_t *bmz8, cmph_uint32 v,
                                                         cmph_uint8 *biggest_g_value,
                                                         cmph_uint8 *biggest_edge_value,
...
cmph_t *bmz8_new(cmph_config_t *mph, double c)
{
    ...
    do {
        ...
        if (mph->verbosity) {
            fprintf(stderr,
                    "Entering mapping step for mph creation of %u keys with graph sized %u\n",
                    bmz8->m, bmz8->n);
        }
        while (1) {
            int ok;
            ...
            if (graph_node_is_critical(bmz8->graph, i) && (!GETBIT(visited, i))) {
                if (c > 1.14)
                    restart_mapping = bmz8_traverse_critical_nodes(
                        bmz8, i, &biggest_g_value, &biggest_edge_value, used_edges, visited);
                else
                        ...
                        (next_g > *biggest_g_value)) // saving the current g value stored in next_g.
                    {
                        if (nunused_g_values == unused_g_values_capacity) {
                            unused_g_values = (cmph_uint8 *)realloc(
                                unused_g_values,
                                ((size_t)(unused_g_values_capacity + BUFSIZ)) * sizeof(cmph_uint8));
...
cmph_uint32 bmz8_packed_size(cmph_t *mphf)
{
    ...
    return (cmph_uint32)(sizeof(CMPH_ALGO) + hash_state_packed_size(h1_type) +
                         hash_state_packed_size(h2_type) + 2 * sizeof(cmph_uint32) +
                         sizeof(cmph_uint8) + sizeof(cmph_uint8) * data->n);
}
