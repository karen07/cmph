...
const cmph_uint8 bdz_lookup_table[] = {
    ...
    3, 3, 3, 2, 3, 3, 3, 2, 3, 3, 3, 2, 2, 2, 2, 1, 2, 2, 2, 1, 2, 2, 2, 1, 2, 2, 2, 1, 1, 1, 1, 0
};
...
cmph_t *bdz_new(cmph_config_t *mph, double c)
{
    ...
    fprintf(stdout, "%u\t%.2f\t%u\t%.4f\t%.4f\n", bdz->m, bdz->m / (double)bdz->n, keys_per_bucket,
            construction_time, space_usage / (double)bdz->m);
#endif
...
static void assigning(bdz_config_data_t *bdz, bdz_graph3_t *graph3, bdz_queue_t queue)
{
    ...
    for (i = nedges - 1; i + 1 >= 1; i--) {
        ...
        DEBUGP("B:%u %u %u -- %u %u %u edge %u\n", v0, v1, v2, GETVALUE(bdz->g, v0),
               GETVALUE(bdz->g, v1), GETVALUE(bdz->g, v2), curr_edge);
...
static inline cmph_uint32 rank(cmph_uint32 b, cmph_uint32 *ranktable, cmph_uint8 *g,
                               cmph_uint32 vertex)
{
